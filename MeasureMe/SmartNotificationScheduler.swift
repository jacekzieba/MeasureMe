...
struct SmartNotificationScheduler {
    ...
    func bestCandidate(
        ...
        lastNotifiedMetric: String? = nil,
        lastLogTimestamp: TimeInterval? = nil
    ) -> Candidate? {
        ...
        let resolvedLastNotifDate: Date? = lastNotificationDate ?? {
            let ts = settings.double(forKey: AppSettingsKeys.Notifications.smartLastNotificationDate)
            return ts > 0 ? Date(timeIntervalSince1970: ts) : nil
        }()
        if let last = resolvedLastNotifDate, now.timeIntervalSince(last) < 86400 {
            return nil
        }

        // Skip if user logged anything today
        let startOfToday = calendar.startOfDay(for: now)
        let logTS = lastLogTimestamp ?? settings.snapshot.notifications.lastLogDate
        if logTS > 0 && Date(timeIntervalSince1970: logTS) >= startOfToday {
            return nil
        }

        // Rotation: prefer different metric than last notified
        let lastMetricRaw = lastNotifiedMetric ?? settings.string(forKey: AppSettingsKeys.Notifications.smartLastNotifiedMetric)

        let sorted = candidates.sorted { $0.reason < $1.reason }

        // Try to pick one that's different from last notified metric
        if let lastMetric = lastMetricRaw, sorted.count > 1 {
        ...
        for kindRaw in activeMetricKinds {
            ...
            candidates.append(Candidate(
                ...
                body: AppLocalization.string(
                    "notification.smart.stale.body",
                    metricTitle as CVarArg,
                    daysSince as CVarArg
                ),
                fireDate: now // will be overridden
            ))
        }
